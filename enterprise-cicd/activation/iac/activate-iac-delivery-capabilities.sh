...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --apply)
      ...
      ;;
    *)
      echo "usage: $0 --environment dev|test|prod [--apply]" >&2
      exit 2
      ;;
  esac
done
...
log() {
  printf '[iac-delivery-%s] %s\n' "$ENVIRONMENT" "$*"
}

fail() {
  printf '[iac-delivery-%s][ERROR] %s\n' "$ENVIRONMENT" "$*" >&2
  exit 1
}

for dependency in "$EDGE_SCRIPT" "$IAM_SCRIPT"; do
...
This activates two isolated protected Apply planes:
...
python3 - "$EDGE_RESULT" "$IAM_RESULT" "$RESULT_FILE" "$ENVIRONMENT" <<'PY'
import json
import sys
from pathlib import Path

edge_path, iam_path, result_path, environment = sys.argv[1:]
...
assert edge["genericNetworkContributor"] is False
assert edge["peeringWrite"] is False
assert edge["natGatewayWrite"] is False
assert edge["applicationGatewayWrite"] is False
assert edge["vpnConnectionWrite"] is False
assert edge["roleAssignmentWrite"] is False
...
assert iam["subscriptionRootTargetAllowedByRenderer"] is False
assert iam["ownerAllowed"] is False
assert iam["contributorAllowed"] is False
assert iam["userAccessAdministratorAllowed"] is False
assert iam["arbitraryRoleAssignmentAllowed"] is False
